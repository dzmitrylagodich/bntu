<?php
    ...
    $fin = mysqli_query($conn, "SELECT * FROM `Finance` WHERE `MoneyID` = '$MoneyID'");
...
<body>
 <h2 style="display: flex; align-items: center; justify-content: center;">Обновить</h2>
    ...
    <form action="../admin/Finance/update.php" method="post">
        <p>ID</p>
        <input type="hidden" name="MoneyID" value="<?= $fin['MoneyID'] ?>">
        <p>Name</p>
        <textarea name="Finance"><?= $fin['Finance'] ?></textarea><br><br>
        <p>Name</p>
        <textarea name="Price"><?= $fin['Price'] ?></textarea><br><br>
